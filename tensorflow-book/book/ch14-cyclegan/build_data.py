...
def data_reader(input_dir, shuffle=True):
    ...
    if shuffle:
        shuffle_index = list(range(len(file_paths)))
        random.seed(12345)
        random.shuffle(shuffle_index)
        file_paths = [file_paths[i] for i in shuffle_index]
    return file_paths
...
def _convert_to_example(filename, image_buffer):
  ...
  """
    return tf.train.Example(features=tf.train.Features(feature={
        'image/file_name': _bytes_feature(tf.compat.as_bytes(os.path.basename(filename))),
...
def data_writer(input_dir, output_file):
    ...
    with tf.python_io.TFRecordWriter(output_file) as writer:
        for i, fp in enumerate(file_paths):
            with tf.gfile.FastGFile(fp, 'rb') as f:
                image_data = f.read()

            example = _convert_to_example(fp.split('/')[-1], image_data)
