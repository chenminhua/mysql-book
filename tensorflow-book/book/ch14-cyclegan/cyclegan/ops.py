...
def c7s1_k(input, k, reuse=False, norm='instance', activation='relu', is_training=True, name='c7s1_k'):
    """
    A 7 * 7 Conv-BatchNorm-ReLU layer with k filters and stride 1
    :param input:  4D tensor
    :param k:   number of filters (output depth)
    :param reuse:
    :param norm:
    :param activation:
    :param is_training:
    ...
    with tf.variable_scope(name, reuse=reuse):
        filters = _weights('weights', shape=[7, 7, input.get_shape()[3], k])
        padded = tf.pad(input, [[0, 0], [3, 3], [3, 3], [0, 0]], 'REFLECT')
        conv = tf.nn.conv2d(padded, filters,
...
def dk(input, k, reuse=False, norm='instance', is_training=True, name=None):
    """
    3 * 3 conv-batchnorm-relu layer with k filters and strides
    :param input:
    :param k:
    :param reuse:
    :param norm:
    ...
    """
    with tf.variable_scope(name, reuse=reuse):
        filters = _weights("weights", shape=[3, 3, input.get_shape()[3], k])
        conv = tf.nn.conv2d(input, filters,
                            strides=[1, 2, 2, 1], padding='SAME')
        normalized = _norm(conv, is_training, norm)
        return tf.nn.relu(normalized)


def Rk(input, k, reuse=False, norm='instance', is_training=True, name=None):
    """
    A residual block that contains two 3 * 3 conv layers with the same number of filters on both layer
    :param input:
    :param k:
    :param reuse:
    :param norm:
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        with tf.variable_scope('layer2', reuse=reuse):
            weights2 = _weights('layer2', shape=[3, 3, relu1.get_shape()[3], k])
...
def _weights(name, shape, mean=0.0, stddev=0.02):
    return tf.get_variable(name, shape, initializer=tf.random_normal_initializer(mean=mean, stddev=stddev))
